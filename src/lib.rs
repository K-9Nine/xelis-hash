...
#[derive(Debug, Clone)]
pub struct Input {
    data: [Bytes8Alignment; BYTES_ARRAY_INPUT / 8],
}

impl Default for Input {
    fn default() -> Self {
        Self {
            data: [Bytes8Alignment([0; 8]); BYTES_ARRAY_INPUT / 8],
        }
    }
}
...
pub fn xelis_hash(input: &mut [u8], scratch_pad: &mut [u64; MEMORY_SIZE]) -> Result<Hash, Error> {
    ...
    for _ in 0..ITERS {
        for chunk in small_pad.chunks_exact(SLOT_LENGTH) {
            // Initialize indices
            ...
            for slot_idx in (0..SLOT_LENGTH).rev() {
                ...
                #[cfg(feature = "nightly")]
                {
                    ...
                    for k in (0..SLOT_LENGTH).step_by(16) {
                        ...
                        let values = u32x16::from_array(unsafe { read_unaligned(&chunk[k] as *const u32 as *const [u32; 16]) });
                    ...
                    if slots[index] >> 31 == 0 {
                        sum_buffer[index % 16] -= chunk[index];
                    } else {
                        sum_buffer[index % 16] += chunk[index];
                    }
    ...
    mem_buffer_a.copy_from_slice(&scratch_pad[addr_a as usize % MEMORY_SIZE..][..BUFFER_SIZE]);
    mem_buffer_b.copy_from_slice(&scratch_pad[addr_b as usize % MEMORY_SIZE..][..BUFFER_SIZE]);
    ...
    for i in 0..SCRATCHPAD_ITERS {
        ...
        for (j, (&a, &b)) in mem_buffer_a.iter().zip(&mem_buffer_b).enumerate().take(HASH_SIZE) {
            // more branching
        ...
        if index < 4 {
            let (left, _) = final_result.split_at_mut(index * 8);
            left.copy_from_slice(&result.to_be_bytes()[..(SCRATCHPAD_ITERS - i) * 8]);
        }
    }
